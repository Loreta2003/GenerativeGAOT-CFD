...
def run_arg(arg):
    ...
    t = Trainer(arg)
    if arg.setup["train"]:
        if arg.model["name"] == "goat2d_fx" or arg.model["name"] == "goat2d_vx":
            project_name = "gaot"
        else:
            project_name = "gaot_gencfd"

        wandb.init(
            project = project_name,
            name=arg.dataset["name"],
            mode = "online",
        )
        if arg.setup["ckpt"]:
            t.load_ckpt()
    ...
    if arg.setup["test"]:
        t.load_ckpt()
        #t.run_eval_loop(ndim = 2 if arg.model["name"] == "denoising_gaot2d_vx" else ValueError("Only 2D models are supported for now."))
        if arg.setup["use_variance_test"]:
            t.variance_test()
        else:
            t.test()
            # t.init_dataset(t.dataset_config)
            # t.val_loader = t.val_loader.to(device = )
            # t.validate(t.val_loader) #This should be test, but currently we are debugging validate function
