...
class OptimizerargsConfig:
    lr: float = 1e-4                    # Initial learning rate ## CHANGED FROM 1e-3 to 1e-4
    ...
    # for mix scheduler
    max_lr: float = 1e-3                # Maximum learning rate for the cosine annealing phase ###CHANGED FROM 1e-2 to 1e-3
    min_lr: float = 1e-6                # Minimum learning rate for the cosine annealing phase. ###CHANGED FROM 1e-5 to 1e-6
...
class AdamOptimizer:
    ...
    def __init__(self, params, config):
        ...
        if config.scheduler == 'step':
            ...
            warmup_epochs = int(0.15 * self.epoch) ##CHANGED FROM 0.02 to 0.15
            cosine_epochs = int(0.80 * self.epoch) ##CHANGED FROM 0.96 to 0.80
        ...
        elif config.scheduler == 'mix_for_diffusion':
            warmup_epochs = int(0.15 * self.epoch) 
            cosine_epochs = int(0.80 * self.epoch) 
            exp_decay_epochs = self.epoch - warmup_epochs - cosine_epochs
            if warmup_epochs == 0:
                warmup_epochs = 1
                cosine_epochs -= 1
    ...
    def optimize(self, trainer: 'TrainerBase',
        ...
        for epoch in range(self.epoch):
            ...
            for batch_idx, batch in enumerate(trainer.train_loader):
                ...
                if trainer.use_ema:
                    train_loss, ema_loss = trainer.train_step(batch, global_step = epoch * len(trainer.train_loader) + batch_idx)
                else:
                    train_loss = trainer.train_step(batch, global_step = epoch * len(trainer.train_loader) + batch_idx)
                global_step = epoch * len(trainer.train_loader) + batch_idx
                self.lr_list.append((global_step, self.optimizer.param_groups[0]['lr']))
                train_loss.backward()
            ...
            if (epoch + 1) % self.eval_every_eps == 0:
                ...
                if trainer.use_ema:
                    train_ema_loss = total_ema_loss / len(trainer.train_loader)
                    ema_losses.append(train_ema_loss)
        ...
        for epoch in range(self.epoch):
            ...
            for batch_idx, batch in enumerate(trainer.train_loader):
                ...
                if getattr(trainer, "ema_model", None) is not None:
                    with torch.no_grad():
                        tgt = trainer.denoising_model.denoiser if trainer.model_config.name == "denoising_gaot2d_vx" else trainer.model
                        if hasattr(tgt, "module"):  # DDP unwrap
                            tgt = tgt.module
                        trainer.ema_model.update_parameters(tgt)

                total_loss += train_loss.detach()
                if trainer.use_ema:
                    total_ema_loss += ema_loss.detach()
            ...
            if (epoch + 1) % self.eval_every_eps == 0:
                ...
                if trainer.use_ema:
                    train_ema_loss = total_ema_loss.cpu().item() / len(trainer.train_loader)
                    ema_losses.append(train_ema_loss)
                losses.append(train_loss)
                epochs.append(epoch)
                val_loss = trainer.validate(trainer.val_loader)
