...
from src.diffusion import diffusion, guidance
from src.solvers import sde
...
def denoiser2score(denoise_fn: DenoiseFn, scheme: diffusion.Diffusion) -> ScoreFn:
    ...
    def _score(xcoord: Tensor, latent_tokens_coord: Tensor, pndata: Tensor, query_coord: Tensor, sigma: float | Tensor, time: float | Tensor, encoder_nbrs: Optional[list]=None, decoder_nbrs: Optional[list]=None) -> Tensor:
        # Reference: eq. 74 in Karras et al. (https://arxiv.org/abs/2206.00364).
        scale = scheme.scale(scheme.sigma.inverse(sigma))
        pndata_hat = pndata / scale
        target = denoise_fn(xcoord, latent_tokens_coord, pndata_hat, query_coord, sigma, encoder_nbrs, decoder_nbrs)
        return (target - pndata_hat) / (scale * sigma**2)
...
def denoise_fn_output(
    ...
    """Depending on the task 'target' compute the result of the
    denoise_fn. If target is None, the denoise_fn is called
    without the target argument, otherwise it is called with the target
    argument.
    """

    if target is None:
        return denoise_fn(xcoord = xcoord, 
...
class Sampler:
    ...
    def generate(
        ...
        target: Tensor = None,
        xcoord: Tensor = None,
        latent_tokens_coord: Tensor = None,
        query_coord: Tensor = None,
        ...
        if self.apply_denoise_at_end:
            ...
            samples = denoise_fn_output(
                ...
                pndata = samples / self.scheme.scale(self.tspan[-1]),
