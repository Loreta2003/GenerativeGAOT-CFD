...
class ConditionedNorm(nn.Module):
    ...
    def __init__(self,
                 ...
                 hidden_size:int):
        super().__init__()
        self.mlp_scale = MLP(input_size, #####CHANGE FROM THE ORIGINAL TO 2 TIMES * WHEN THERE IS TIME CONDITIONING
                             output_size,
                             hidden_size,
                             num_layers=2,
                             activation="none")
        self.mlp_bias = MLP(input_size, #####CHANGE FROM THE ORIGINAL TO 2 TIMES * WHEN THERE IS TIME CONDITIONING
                            output_size,
                            hidden_size,
                            num_layers=2,
                            activation="none")
