...
class DenoisingModelGAOT2DFX(BaseModel):
    ...
    def loss_fn(self, batch: dict, latent_x_array, encoder_nbrs, decoder_nbrs, global_step: int | None = None, mutables: Optional[dict] = None):
                ...
                denoised = torch.stack(
                    [
                        denoise_fn(
                            ...
                            cond = dict(xcoord = x_coords, latent_tokens_coord = latent_x_array, query_coord = query_x_array[i], encoder_nbrs = None, decoder_nbrs = None),
                            time = time_inputs[i])
                ...
                denoised = torch.stack(
                    [
                        denoise_fn(
                            ...
                            y = noised[i], 
                            sigma = sigma[i].unsqueeze(0), 
                            cond = dict(xcoord = x_coords, latent_tokens_coord = latent_x_array, query_coord = None, encoder_nbrs = None, decoder_nbrs = None),
                            time = time_inputs[i]
                        )
                        for i in range(self.num_eval_noise_levels)
                    ]
