...
from src.utils.eval_utils import summarize_metric_results

from src.diffusion.samplers import Sampler
    ...
    *,
    ...
    if compute_metrics:
        ...
        for i in range(n_iter):
            ...
            x_sample = batch[0]
            y_sample = batch[1]
            coord_sample = batch[2]
            encoder_graph_sample = batch[3]
            decoder_graph_sample = batch[4]
            ...
            gen_samples = sampler.generate(
                ...
                cond = dict(
                    ...
                    encoder_graph=encoder_graph_sample,
                    decoder_graph=decoder_graph_sample,
            ...
            # u = u * mask
            stats_recorder.update_step(gen_samples, y_sample)
    ...
    if visualize or save_gen_samples:
        ...
        if save_gen_samples and (local_rank == 0 or local_rank == -1):
            ...
            if ndim == 3:
                # plot 2D results
                # Denormalize and visualize the results
                gen_samples_de_norm = gen_samples * u_std.to(device) + u_mean.to(device)
                y_sample_de_norm = y_sample * u_std.to(device) + u_mean.to(device)
                x_sample_de_norm = x_sample * c_std.to(device) + c_mean.to(device)
                fig = plot_estimates(
                    ...
                    # names = metadata.names['c'],
                    # symmetric = metadata.signed['u'])
                
                fig.savefig(save_dir,dpi=300,bbox_inches="tight", pad_inches=0.1)
                plt.close(fig)
